...
def test_multithreaded_kneighbors_no_root():
    ...
    def search_batch(i):
        ...
        kd_tree = pynanoflann.KDTree(n_neighbors=1, metric='L2', leaf_size=20, root_dist=False)
        kd_tree.fit(pts_target)
...
def test_multithreaded_radius_no_root():
    index = np.random.rand(40_000, 3)
    query = np.random.rand(20_000, 3)

    kd_tree = pynanoflann.KDTree(metric="L2", radius=0.1, root_dist=False)
    kd_tree.fit(index)
